...
//把字符串转为数组(对象)
let dataArr = JSON.parse(dataList);
// console.log(dataArr); 
//获取元素
let financeList = getEle("#financeList")
let businessList = getEle("#businessList")
let governmentList = getEle("#governmentList")
// 根据数据，渲染页面
dataArr.forEach(function (obj, index) {
    ...
    //向新的li标签中添加内容
    newLi.innerHTML = `
    <a href="./customer-details.html" class="item-link"></a>
    <img class="pic" src="./images/${obj.img}" alt="">
    <p class="item-name">
         ${obj.name}
    </p>
    <p class="item-desc">
...
`
    // console.log(newLi);
    //添加样式
    newLi.setAttribute("class", "case-item fl");
    //显示到页面上
    /*  if (obj.category === "finance") {
         financeList.append(newLi)
     } else if (obj.category === "business") {
         businessList.append(newLi)
     } else {
         governmentList.append(newLi)
     } */
...
$(".aside .nav-item").click(function () {
    ...
    // 找到对应的楼层区域距离文档顶部的距离
    let st = $(".floor").eq(index).offset().top;
    //把上成的距离赋值给滚动条
    // $(window).scrollTop( st);
